...
export async function doBootstrap({
    ...
    const validatorAppCallTxn = algosdk.makeApplicationOptInTxnFromObject({
        ...
        appArgs: [BOOTSTRAP_ENCODED, algosdk.encodeUint64(asset1ID), algosdk.encodeUint64(asset2ID)],
        suggestedParams,
    });

    const liquidityTokenCreateTxn = algosdk.makeAssetCreateTxnWithSuggestedParamsFromObject(<any>{
        from: poolLogicSig.addr,
    ...
    const asset2Optin = asset2ID === 0 ? null : algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject({
        from: poolLogicSig.addr,
        to: poolLogicSig.addr,
        assetIndex: asset2ID,
        amount: 0,
    ...
    const minBalance = 100000 // min account balance
        ...
        + 100000 + (25000+3500)*VALIDATOR_APP_SCHEMA.numLocalInts + (25000+25000)*VALIDATOR_APP_SCHEMA.numLocalByteSlices; // min balance to opt into validator app
