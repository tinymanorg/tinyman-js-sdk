 ...
 */
export async function getPoolInfo(client: any, pool: {
    validatorAppID: number,
    asset1ID: number,
    asset2ID: number,
}): Promise<PoolInfo> {
...
export async function createPool(
    ...
    const { liquidityTokenID } = await doBootstrap({
        client,
        poolLogicSig,
        validatorAppID,
        asset1ID,
...
export async function getPoolReserves(client: any, pool: PoolInfo): Promise<{
    ...
    for (const app of appsLocalState) {
        ...
        const outstandingAsset1Key = fromByteArray(joinUint8Arrays([
            OUTSTANDING_ENCODED, algosdk.encodeUint64(pool.asset1ID)
        ]));
        const outstandingAsset2Key = fromByteArray(joinUint8Arrays([
            OUTSTANDING_ENCODED, algosdk.encodeUint64(pool.asset2ID)
        ]));
        const outstandingLiquidityTokenKey = fromByteArray(joinUint8Arrays([
            OUTSTANDING_ENCODED, algosdk.encodeUint64(pool.liquidityTokenID!)
        ]));
...
export async function getAccountExcess({
    ...
    for (const app of appsLocalState) {
        ...
        const excessAsset1Key = fromByteArray(joinUint8Arrays([
            EXCESS_ENCODED, algosdk.encodeUint64(pool.asset1ID)
        ]));
        const excessAsset2Key = fromByteArray(joinUint8Arrays([
            EXCESS_ENCODED, algosdk.encodeUint64(pool.asset2ID)
        ]));
        const excessLiquidityTokenKey = fromByteArray(joinUint8Arrays([
            EXCESS_ENCODED, algosdk.encodeUint64(pool.liquidityTokenID!)
        ]));
